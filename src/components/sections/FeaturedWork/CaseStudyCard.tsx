...
const gradients = [
  "from-violet-200/70 to-purple-100/50",
  "from-cyan-200/70 to-teal-100/50",
  "from-amber-200/70 to-orange-100/50",
];
    ...
    >
      ...
      >
        ...
        <div className="absolute bottom-3 left-3">
          <span className="px-2.5 py-1 rounded-lg bg-[#FDFAF5]/80 backdrop-blur-sm border border-white/[0.1] text-xs font-bold text-[#22D3EE]">
            {cs.metrics[0]?.value}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 p-6 flex-1">
        ...
        <div className="flex flex-wrap gap-4">
          {cs.metrics.map((m) => (
            <div key={m.label} className="flex flex-col">
              ...
              <span className="text-xs text-[#6B7280] mt-0.5">{m.label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="mt-auto">
          <Link
            href={cs.href}
            className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#22D3EE] hover:text-[#1A1A1A] transition-colors group"
          >
            View Case Study
