  ...
  return (
    <>
      ...
      <article className="mx-auto max-w-5xl py-2 pb-12 px-4 sm:px-6 lg:px-8">
        {!article.coverImage && (
          <header className="rounded-3xl border border-slate-200 dark:border-stone-300 bg-white dark:bg-[#EDE8DF] shadow-sm p-6 sm:p-10">
            {article.source && (
            ...
            <div className="flex items-center gap-4 mb-8 text-gray-500 dark:text-gray-500 text-sm">
              {article.publishedAt && (
        ...
        {article.excerpt && (
          <p className="text-lg sm:text-xl text-slate-600 dark:text-gray-600 leading-relaxed mt-8 mb-8 font-medium border-l-4 border-blue-500 dark:border-blue-400 pl-6 italic">
            {article.excerpt}
          </p>
        )}

        <div className="rounded-3xl border border-slate-200 dark:border-stone-300 bg-white dark:bg-[#EDE8DF] shadow-sm p-6 sm:p-10">
          <div
          ...
          className="prose prose-slate prose-lg sm:prose-xl max-w-none
            ...
            prose-p:text-base sm:prose-p:text-lg prose-p:leading-relaxed prose-p:text-slate-700 dark:prose-p:text-gray-600
            prose-li:text-base sm:prose-li:text-lg prose-li:text-slate-700 dark:prose-li:text-gray-600
            prose-strong:text-slate-900 dark:prose-strong:text-white
            prose-a:text-blue-600 dark:prose-a:text-blue-400 prose-a:no-underline hover:prose-a:underline
            prose-code:text-slate-900 dark:prose-code:text-gray-800
            prose-pre:bg-[#F5F0E8] dark:prose-pre:bg-slate-950
            prose-blockquote:text-slate-700 dark:prose-blockquote:text-gray-600 prose-blockquote:border-slate-300 dark:prose-blockquote:border-stone-300
        ...
        <div className="mt-10 pt-8 border-t border-slate-200 dark:border-stone-300">
      ...
      {/* More Case Studies */}
      {moreNews.length > 0 && (
        <section className="py-16 bg-white dark:bg-[#F5F0E8] border-t border-slate-200 dark:border-stone-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {moreNews.map((news) => (
                <Link key={news.slug} href={`/news/${news.slug}`} className="group">
                  <article className="rounded-2xl overflow-hidden border border-slate-200 dark:border-stone-300 hover:shadow-lg transition-all hover:-translate-y-1 bg-white dark:bg-[#EDE8DF]">
