...
export default function TableOfContents({ html }: Props) {
  ...
  return (
    <nav className="toc rounded-2xl border border-slate-200 dark:border-stone-300 bg-white dark:bg-[#EDE8DF] shadow-sm p-5 sm:p-6">
      ...
      <ul className="mt-3 space-y-2">
        {headings.map((h) => (
          ...
          >
            <a
              href={`#${h.id}`}
              className="text-slate-600 dark:text-gray-600 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors"
            >
              {h.text}
            </a>
          </li>
