...
export default function LandingFooter() {
  ...
  return (
    <footer className={`py-12 mt-12 ${isDark ? 'bg-[#F5F0E8] text-white' : 'bg-[#FDFAF5] text-slate-900'}`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="space-y-4">
          <Logo showText={false} iconSize={48} darkText={!isDark} />
          <h4 className="font-bold">Services</h4>
          <ul className="space-y-2">
            <li><a href="/services" className={`transition-colors ${isDark ? 'text-gray-600 hover:text-gray-900' : 'text-slate-600 hover:text-emerald-600'}`}>All Services</a></li>
            <li><a href="/services/google-ads" className={`transition-colors ${isDark ? 'text-gray-600 hover:text-gray-900' : 'text-slate-600 hover:text-emerald-600'}`}>Google Ads</a></li>
            <li><a href="/services/seo-local-search" className={`transition-colors ${isDark ? 'text-gray-600 hover:text-gray-900' : 'text-slate-600 hover:text-emerald-600'}`}>Local SEO</a></li>
          </ul>
        </div>
        <div className="space-y-4">
          <h4 className="font-bold">Services</h4>
          <ul className="space-y-2">
            <li><a href="/services" className={`transition-colors ${isDark ? 'text-gray-600 hover:text-gray-900' : 'text-slate-600 hover:text-emerald-600'}`}>All Services</a></li>
            <li><a href="/services/seo-local-search" className={`transition-colors ${isDark ? 'text-gray-600 hover:text-gray-900' : 'text-slate-600 hover:text-emerald-600'}`}>Healthcare SEO</a></li>
          </ul>
        </div>
        <div className="space-y-4">
      ...
      <div className={`mt-8 text-center text-sm ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>
        © {new Date().getFullYear()} Shree Krishna Gauli. All rights reserved.
      </div>
    </footer>
  );
}
