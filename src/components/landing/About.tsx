...
export default function About() {
  ...
  return (
    <section className={`py-20 ${isDark ? 'bg-[#F5F0E8] text-white' : 'bg-white text-slate-900'}`}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 className={`text-3xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>We Are Architects of Bold Products</h2>
        <p className={`mt-4 text-lg ${isDark ? 'text-gray-600' : 'text-slate-600'}`}>Artists, Strategists, Innovators</p>
        <p className={`mt-6 max-w-2xl mx-auto ${isDark ? 'text-gray-500' : 'text-slate-600'}`}>
